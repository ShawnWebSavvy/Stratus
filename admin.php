		...
		case 'verification':
			...
			switch ($_GET['sub_view']) {
				case '':
					...
					// get data
					$get_rows = $db->query("SELECT verification_requests.*, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture, picture_photo.source as user_picture_full, pages.page_name, pages.page_title, pages.page_picture FROM verification_requests LEFT JOIN users ON verification_requests.node_type = 'user' AND verification_requests.node_id = users.user_id LEFT JOIN posts_photos as picture_photo ON users.user_picture_id = picture_photo.photo_id LEFT JOIN pages ON verification_requests.node_type = 'page' AND verification_requests.node_id = pages.page_id WHERE status = '0'") or _error("SQL_ERROR");
					if ($get_rows->num_rows > 0) {
						while ($row = $get_rows->fetch_assoc()) {
							/* get node */
							if ($row['node_type'] == "user") {
								...
								if (!empty($row['user_picture'])) {
									...
									if ($row['user_picture_full'] == "") {
										$row['user_picture_full'] = 'content/themes/default/images/user_defoult_img.jpg';
									}
									$row['user_picture'] = 'includes/wallet-api/image-exist-api.php?userPicture=' . $row_user_picture . '&userPictureFull=' . $system['system_uploads'] . "/" . $row['user_picture_full'];
								}
								$row['color'] = 'primary';
							} elseif ($row['node_type'] == 'page') {
								$row['page_picture'] = get_picture($row['page_picture'], 'page');
