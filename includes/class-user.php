...
class User
{
    ...
    public function __construct()
    {
        ...
        if (isset($_COOKIE[$this->_cookie_user_id]) && isset($_COOKIE[$this->_cookie_user_token])) {
            ...
            if ($get_user->num_rows > 0) {
                ...
                $this->_data['user_picture'] = 'includes/wallet-api/image-exist-api.php?userPicture=' . $this->_data['user_picture'] . '&userPictureFull=' . $system['system_uploads'] . '/' . $this->_data['user_picture_full'] . '&type=1';
        ...
        if ($get_friends->num_rows > 0) {
            while ($friend = $get_friends->fetch_assoc()) {
                ...
                if (!empty($friend['user_picture'])) {
                    ...
                    $friend['user_picture'] = 'includes/wallet-api/image-exist-api.php?userPicture=' . $friend['user_picture'] . '&userPictureFull=' . $system['system_uploads'] . '/' . $friend['user_picture_full'];
                }
    ...
    public function get_friend_requests($offset = 0, $last_request_id = null)
    {
        ...
        if ($last_request_id !== null) {
            $get_requests = $db->query(sprintf("SELECT friends.id, friends.user_one_id as user_id, users.user_name, users.user_firstname, posts_photos.source as user_picture_full, users.user_picture_id, users.user_lastname, users.user_gender, users.user_picture FROM friends INNER JOIN users ON friends.user_one_id = users.user_id LEFT JOIN posts_photos ON users.user_picture_id = posts_photos.photo_id WHERE friends.status = 0 AND friends.user_two_id = %s AND friends.id > %s ORDER BY friends.id DESC", secure($this->_data['user_id'], 'int'), secure($last_request_id, 'int'))) or _error("SQL_ERROR_THROWEN");
        } else {
            $get_requests = $db->query(sprintf("SELECT friends.id, friends.user_one_id as user_id, users.user_name, users.user_firstname, posts_photos.source as user_picture_full, users.user_picture_id, users.user_lastname, users.user_gender, users.user_picture FROM friends INNER JOIN users ON friends.user_one_id = users.user_id LEFT JOIN posts_photos ON users.user_picture_id = posts_photos.photo_id WHERE friends.status = 0 AND friends.user_two_id = %s ORDER BY friends.id DESC LIMIT %s, %s", secure($this->_data['user_id'], 'int'), secure($offset, 'int', false), secure($system['max_results'], 'int', false))) or _error("SQL_ERROR_THROWEN");
        }
        if ($get_requests->num_rows > 0) {
            while ($request = $get_requests->fetch_assoc()) {
                ...
                $request['user_picture'] = 'includes/wallet-api/image-exist-api.php?userPicture=' . $request['user_picture'] . '&userPictureFull=' . $system['system_uploads'] . '/' . $request['user_picture_full'];

                $request['mutual_friends_count'] = $this->get_mutual_friends_count($request['user_id']);
                $requests[] = $request;
            }
        }
        // if ($this->_data['user_id'] == "4") {
        //     // echo "<pre>";
        //     // print_R($requests);
        //     die;
        // }
     ...
     * @return array
     */
    public function get_new_people($offset = 0, $random = false, $page = "LocalHub")
    {
        global $db, $system;
        $results = [];
        ...
        if ($page == "GlobalHub") {
            ...
            $profileName = "users.user_firstname != '' and users.user_picture != '' and ";
        }
        ...
        if (isset($old_people_ids) && count($old_people_ids) > 0) {
            ...
            $where .= "WHERE " . $profileName . " users.user_banned = '0'";
        }
        ...
        if ($page == "GlobalHub") {
            ...
            // if ($this->_data['user_id'] == "3441") {
            //     echo $myquery;
            //     echo $offset;
            //     die;
            // }
            $get_users = $db->query($myquery) or _error("SQL_ERROR_THROWEN");
        } else {
            $offset *= $system['min_results'];
            if ($random) {
                $myquery =  sprintf("SELECT * FROM (SELECT users.user_id, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture, picture_photo.source as user_picture_full, users.user_subscribed, country.country_name as user_country_name, users.user_verified, (%s * acos(cos(radians(%s)) * cos(radians(user_latitude)) * cos(radians(user_longitude) - radians(%s)) + sin(radians(%s)) * sin(radians(user_latitude))) ) AS distance FROM users LEFT JOIN posts_photos as picture_photo ON users.user_picture_id = picture_photo.photo_id left join system_countries as country on country.country_id = users.user_country " . $where . " HAVING distance < %s ORDER BY distance ASC LIMIT %s) tmp ORDER BY RAND() LIMIT %s", secure($unit, 'int'), secure($this->_data['user_latitude']), secure($this->_data['user_longitude']), secure($this->_data['user_latitude']), secure($distance, 'int'), secure($system['max_results'] * 2, 'int', false), secure($system['min_results'], 'int', false));
            } else {
                $myquery = sprintf("SELECT users.user_id, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture, picture_photo.source as user_picture_full, users.user_subscribed, country.country_name as user_country_name, users.user_verified, (%s * acos(cos(radians(%s)) * cos(radians(user_latitude)) * cos(radians(user_longitude) - radians(%s)) + sin(radians(%s)) * sin(radians(user_latitude))) ) AS distance FROM users LEFT JOIN posts_photos as picture_photo ON users.user_picture_id = picture_photo.photo_id left join system_countries as country on country.country_id = users.user_country " . $where . " HAVING distance < %s ORDER BY distance ASC LIMIT %s, %s", secure($unit, 'int'), secure($this->_data['user_latitude']), secure($this->_data['user_longitude']), secure($this->_data['user_latitude']), secure($distance, 'int'), secure($offset, 'int', false), secure($system['min_results'], 'int', false));
            }
            $get_users = $db->query($myquery) or _error("SQL_ERROR_THROWEN");
        }
        ...
        if ($get_users->num_rows > 0) {
            while ($user = $get_users->fetch_assoc()) {
                ...
                //                }
                $user['user_picture_full'] = $system['system_uploads_url'] . "/" . $user['user_picture_full'];
     ...
     * @return array
     */
    public function search_users_global($distance, $keyword, $gender, $relationship, $status)
    {
        global $db, $system;
        $results = [];
        $offset *= $system['max_results'];
        ...
        /* make a list from old people */
        $old_people_ids_list = implode(',', $old_people_ids);
        $where .= sprintf("WHERE users.user_banned = '0' AND global_profile.user_firstname != '' AND users.user_id NOT IN (%s)", $old_people_ids_list);
        if ($system['activation_enabled']) {
            $where .= " AND users.user_activated = '1'";
        }
        /* keyword */
        if ($keyword) {
            $where .= sprintf(' AND (users.user_name LIKE %1$s OR global_profile.user_firstname LIKE %1$s OR global_profile.user_lastname LIKE %1$s OR CONCAT(global_profile.user_firstname,  " ", global_profile.user_lastname) LIKE %1$s)', secure($keyword, 'search'));
        }
        /* gender */
        if ($gender != "any") {
            $where .= " AND users.user_gender = '$gender'";
        ...
        /* relationship */
        if ($relationship != "any") {
            $where .= " AND global_profile.user_relationship = '$relationship'";
        }
        /* status */
        if ($status != "any") {
            if ($status == "online") {
        ...
        /* get users */
        $query = sprintf("SELECT global_profile.*, users.user_id, users.user_group, users.user_name, users.user_email, users.knox_user_id, users.user_email_verified, users.user_email_verification_code, users.user_phone, users.user_phone_verified, users.user_phone_verification_code, users.user_password, users.user_two_factor_enabled, users.user_two_factor_type, users.user_two_factor_key, users.user_two_factor_gsecret, users.user_activated, users.user_reseted, users.user_reset_key, users.user_subscribed, users.user_package, users.user_subscription_date, users.user_boosted_posts, users.user_boosted_pages, users.user_started, users.user_verified, users.user_banned, users.user_live_requests_counter, users.user_live_requests_lastid, users.user_live_messages_counter, users.user_live_messages_lastid, users.user_live_notifications_counter, users.user_live_notifications_lastid, users.user_latitude, users.user_longitude, users.user_location_updated, users.user_picture, users.global_user_picture, users.global_user_picture_id, users.user_picture_id, users.global_user_cover_id, users.global_user_cover, users.user_cover, users.user_cover_id, users.user_cover_position, users.user_album_pictures, users.user_album_covers, users.user_album_timeline, users.user_pinned_post, users.user_registered, users.user_last_seen, users.user_first_failed_login, users.user_failed_login_ip, users.user_failed_login_count, users.user_work_title, users.user_work_place, users.user_work_url, users.user_current_city, users.user_hometown, users.user_edu_major, users.user_edu_school, users.user_edu_class, users.user_social_facebook, users.user_social_twitter, users.user_social_youtube, users.user_social_instagram, users.user_social_linkedin, users.user_social_vkontakte, users.user_profile_background, users.user_chat_enabled, users.user_privacy_newsletter, users.user_privacy_poke, users.user_privacy_gifts, users.user_privacy_wall, users.user_privacy_birthdate, users.user_privacy_relationship, users.user_privacy_basic, users.user_privacy_work, users.user_privacy_location, users.user_privacy_education, users.user_privacy_other, users.user_privacy_friends, users.user_privacy_photos, users.user_privacy_pages, users.user_privacy_groups, users.user_privacy_events, users.email_post_likes, users.email_post_comments, users.email_post_shares, users.email_wall_posts, users.email_mentions, users.email_profile_visits, users.email_friend_requests, users.facebook_connected, users.facebook_id, users.google_connected, users.google_id, users.twitter_connected, users.twitter_id, users.instagram_connected, users.instagram_id, users.linkedin_connected, users.linkedin_id, users.vkontakte_connected, users.vkontakte_id, users.user_referrer_id, users.user_affiliate_balance, users.user_wallet_balance, users.user_points, users.chat_sound, users.notifications_sound, users.onesignal_user_id, users.user_language, users.globalToken, users.global_user_album_pictures, (%s * acos(cos(radians(%s)) * cos(radians(user_latitude)) * cos(radians(user_longitude) - radians(%s)) + sin(radians(%s)) * sin(radians(user_latitude))) ) AS distance FROM users JOIN global_profile ON global_profile.user_id = users.user_id ", secure($unit, 'int'), secure($this->_data['user_latitude']), secure($this->_data['user_longitude']), secure($this->_data['user_latitude']));
        $query .= $where;
        $query .= sprintf(" HAVING distance < %s ORDER BY distance ASC ", secure($distance, 'int'));
        $get_users = $db->query($query) or _error("SQL_ERROR_THROWEN");
        if ($get_users->num_rows > 0) {
            while ($user = $get_users->fetch_assoc()) {
    ...
    public function connect($do, $id, $uid = null)
    {
        ...
        switch ($do) {
            ...
            case 'poke':
                ...
                /* post new notification */
                $this->post_notification(array('to_user_id' => $id, 'action' => 'poke', 'hub' => "LocalHub", 'node_url' => $this->_data['user_name']));
                break;
    ...
    private function _follow($user_id)
    {
        ...
        /* post new notification */
        $this->post_notification(array('to_user_id' => $user_id, 'hub' => "LocalHub", 'action' => 'follow'));
    }
    ...
    public function get_notifications($offset = 0, $last_notification_id = null)
    {
        ...
        if ($get_notifications->num_rows > 0) {
            while ($notification = $get_notifications->fetch_assoc()) {
                ...
                $notification['user_picture'] = 'includes/wallet-api/image-exist-api.php?userPicture=' . $notification['user_picture'] . '&userPictureFull=' . $system['system_uploads'] . '/' . $notification['user_picture_full'];
                ...
                switch ($notification['action']) {
                    ...
                    case 'follow':
                        ...
                        if ($notification['hub'] == 'GlobalHub') {
                            $notification['url'] = $system['system_url'] . '/global-profile.php?username=' . $notification['user_name'];
                        } else {
                            $notification['url'] = $system['system_url'] . '/' . $notification['user_name'];
                        }
                    ...
                    case 'comment':
                        ...
                        if ($notification['node_type'] == "post") {
                            if ($notification['hub'] == 'GlobalHub') {
                                $notification['url'] = $system['system_url'] . '/global-profile-posts/' . $notification['node_url'] . $notification['notify_id'];
                            } else {
                                $notification['url'] = $system['system_url'] . '/posts/' . $notification['node_url'] . $notification['notify_id'];
                            }
                ...
                /* prepare message */
                if ($notification['action'] != "profile_visit") {
    ...
    public function get_conversation($conversation_id)
    {
        ...
        while ($recipient = $get_recipients->fetch_assoc()) {
            ...
            $recipient['user_picture'] = 'includes/wallet-api/image-exist-api.php?userPicture=' . $recipient['user_picture'] . '&userPictureFull=' . $system['system_uploads'] . '/' . $recipient['user_picture_full'];
        ...
        if ($recipients_num > 1) {
            ...
            if ($recipients_num > 2) {
                ...
                $name_new = "<div class='multiple-recipients-section'> <div class='multiple-recipients-image data-avatar'><div class='left-avatar'><img src='" . $conversation['picture_left'] = $conversation['recipients'][0]['user_picture'] . "'> </div><div class='right-avatar'><img src='" . $conversation['picture_left'] = $conversation['recipients'][1]['user_picture'] . "'> </div></div>";
                $name_new .= "<div class='multiple-recipients-text'>" . $conversation['name'] . " </div></div>";
                $conversation['name_new'] = $name_new;
                $conversation['senderName'] = $conversation['name'];
                $conversation['senderUserImage'] = $conversation['recipients'][0]['user_picture'];
    ...
    public function get_conversation_messages($conversation_id, $offset = 0, $last_message_id = null)
    {
        ...
        while ($message = $get_messages->fetch_assoc()) {
            ...
            $message['user_picture'] = 'includes/wallet-api/image-exist-api.php?userPicture=' . $message['user_picture'] . '&userPictureFull=' . $system['system_uploads'] . '/' . $message['user_picture_full'];
    ...
    public function stop_live_recording($post_id, $agora_uid, $agora_channel_name, $agora_resource_id, $agora_sid)
    {
        ...
        /* update post */
        $db->query(sprintf("UPDATE posts_live SET live_recorded = '1', agora_file = '$file' WHERE post_id = %s", secure($post_id, 'int'))) or _error("SQL_ERROR_THROWEN");
    }
            ...
            foreach ($matches[1] as $username) {
                if ($this->_data['user_name'] != $username && !in_array($username, $done)) {
                    ...
                    if ($get_user->num_rows > 0) {
                        ...
                        $this->post_notification(array('to_user_id' => $_user['user_id'], 'action' => 'mention', 'node_type' => $node_type, 'hub' => "LocalHub", 'node_url' => $node_url, 'notify_id' => $notify_id));
                        $done[] = $username;
                    }
                }
            }
    ...
    public function publisher($args = [])
    {
        ...
        switch ($post['post_type']) {
            ...
            case 'video':
                $videosave = sprintf("INSERT INTO posts_videos (post_id, source, thumbnail) VALUES (%s, %s, %s)", secure($post['post_id'], 'int'), secure($args['video']->source), secure($args['video_thumbnail']));
                $db->query($videosave) or _error("SQL_ERROR_THROWEN");
        ...
        if ($post['in_wall']) {
            $this->post_notification(array('to_user_id' => $post['wall_id'], 'action' => 'wall', 'hub' => "LocalHub", 'node_type' => 'post', 'node_url' => $post['post_id']));
        }

        /* post in_group notification */
        if ($post['in_group'] && !$post['group_approved']) {
            /* send notification to group admin */
            $this->post_notification(array('to_user_id' => $_group['group_admin'], 'action' => 'group_post_pending', 'hub' => "LocalHub", 'node_type' => $_group['group_title'], 'node_url' => $_group['group_name'] . "-[guid=]" . $post['post_id']));
        }

        /* post in_event notification */
        if ($post['in_event'] && !$post['event_approved']) {
            /* send notification to event admin */
            $this->post_notification(array('to_user_id' => $_event['event_admin'], 'action' => 'event_post_pending', 'hub' => "LocalHub", 'node_type' => $_event['event_title'], 'node_url' => $_event['event_id'] . "-[guid=]" . $post['post_id']));
        }
     ...
     * @return array
     */
    public function get_posts_pinned($args = [])
    {
        global $db, $system;
        /* initialize vars */
        $posts = [];
     ...
     * @return array
     */
    public function get_boosted_all($args = [])
    {
        global $db, $system;
        /* initialize vars */
        $posts = [];
        ...
        if (!$this->_logged_in && $query) {
            ...
            /* get viewer posts */
            $me = $this->_data['user_id'];
            $where_query .= "(posts.user_id = $me AND posts.boosted = '1' )";
        ...
        if ($last_post_id != null && $get != 'popular' && $get != 'saved' && $get != 'memories') { /* excluded as not ordered by post_id */
            ...
            $limit_statement = ($get_all) ? "" : sprintf("LIMIT %s, %s", secure($offset, 'int', false), secure($system['max_results'], 'int', false)); /* get_all for cases like download user's posts */
            $get_postsQuery = "SELECT posts.post_id FROM posts " . $where_query . " " . $order_query;
        }
        //echo $get_postsQuery;
        // exit;
        $get_posts = $db->query($get_postsQuery) or _error("SQL_ERROR_THROWEN");
        if ($get_posts->num_rows > 0) {
            while ($post = $get_posts->fetch_assoc()) {
                $Boostedpost = $this->get_post($post['post_id'], true, true); /* $full_details = true, $pass_privacy_check = true */
                if ($Boostedpost) {
     ...
     * @return array
     */
    public function get_posts_all($args = [])
    {
        global $db, $system;
        /* initialize vars */
        $posts = [];
        ...
        $get = !isset($args['get']) ? 'newsfeed' : $args['get'];
        $filter = !isset($args['filter']) ? 'all' : $args['filter'];
        if (!in_array($filter, array('all', '', 'link', 'media', 'live', 'photos', 'map', 'product', 'article', 'poll', 'video', 'audio', 'file'))) {
            _error(400);
        }
        $last_post_id = !isset($args['last_post_id']) ? null : $args['last_post_id'];
        if (isset($last_post_id) && !is_numeric($last_post_id)) {
    ...
    public function who_votes($option_id, $offset = 0)
    {
        ...
        while ($voter = $get_voters->fetch_assoc()) {
            ...
            $voter['user_picture'] = 'includes/wallet-api/image-exist-api.php?userPicture=' . $voter['user_picture'] . '&userPictureFull=' . $system['system_uploads'] . '/' . $voter['user_picture_full'];
            /* get the connection between the viewer & the target */
    ...
    private function _check_post($id, $pass_privacy_check = false, $full_details = true)
    {
        ...
        // echo $this->_data['user_id'];echo"asfasf";
        // echo $post['author_id'];die;
        /* full details */
        if ($full_details) {
            /* check if wall post */
            if ($post['in_wall']) {
    ...
    public function shareCount($post_id)
    {
        ...
        /* get the origin post shares counter */
        $myquery = sprintf("select shares from posts WHERE post_id = %s", secure($post_id, 'int'));
        $countshare = $db->query($myquery) or _error("SQL_ERROR_THROWEN");
        if ($countshare->num_rows > 0) {
            $total_Counts = $countshare->fetch_assoc();
            $totalCounts = $total_Counts['shares'];
        } else {
            $totalCounts = 0;
        }
        return $totalCounts;
    ...
    public function share($post_id, $args)
    {
        ...
        /* post notification */
        $this->post_notification(array('to_user_id' => $author_id, 'action' => 'share', 'hub' => "LocalHub", 'node_type' => 'post', 'node_url' => $post_id));
    }
            ...
            case 'product':
                ...
                switch ($post['post_type']) {
                    ...
                    case 'product':
                        ...
                        //$refresh['callback'] = 'window.location = "' . $system['system_url'] . '/market';
                        break;
                }
                break;
    ...
    public function approve_post($post_id)
    {
        ...
        if ($post['in_group']) {
            ...
            $this->post_notification(array('to_user_id' => $post['author_id'], 'action' => 'group_post_approval', 'hub' => "LocalHub", 'node_type' => $post['group_title'], 'node_url' => $post['group_name']));
        } elseif ($post['in_event']) {
            ...
            /* send notification to the post author */
            $this->post_notification(array('to_user_id' => $post['author_id'], 'action' => 'event_post_approval', 'hub' => "LocalHub", 'node_type' => $post['event_title'], 'node_url' => $post['event_id']));
        }
    }
    ...
    public function get_comments($node_id, $offset = 0, $is_post = true, $pass_privacy_check = true, $post = array(), $top_sorted = false)
    {
        ...
        if ($is_post) {
            ...
            /* get photo comments */
            $get_comments = $db->query(sprintf("SELECT posts_comments.*, posts_photos.source as user_picture_full, users.user_name, users.user_picture_id, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture, users.user_verified, pages.* FROM posts_comments LEFT JOIN users ON posts_comments.user_id = users.user_id AND posts_comments.user_type = 'user' LEFT JOIN posts_photos ON users.user_picture_id = posts_photos.photo_id LEFT JOIN pages ON posts_comments.user_id = pages.page_id AND posts_comments.user_type = 'page' WHERE NOT (users.user_name <=> NULL AND pages.page_name <=> NULL) AND posts_comments.node_type = 'photo' AND posts_comments.node_id = %s " . $order_query . " LIMIT %s, %s", secure($node_id, 'int'), secure($offset, 'int', false), secure($system['min_results'], 'int', false))) or _error("SQL_ERROR_THROWEN");
        }
        ...
        while ($comment = $get_comments->fetch_assoc()) {
            ...
            if ($comment['user_type'] == "user") {
                ...
                $comment['author_picture'] = 'includes/wallet-api/image-exist-api.php?userPicture=' . $comment['author_picture'] . '&userPictureFull=' . $system['system_uploads'] . '/' . $comment['user_picture_full'];
     ...
     * @return integer
     */
    public function commentCalculate($node_id)
    {
        global $db, $system, $date;
        $calcQuery = sprintf("SELECT count(comment_id) as total_comment FROM `posts_comments` WHERE posts_comments.node_id = %s", secure($node_id, 'int'));
        $get_comment_calc = $db->query($calcQuery) or _error("SQL_ERROR_THROWEN");
        $commentCount = $get_comment_calc->fetch_assoc();
        return $commentCount;
    }
    ...
    public function comment($handle, $node_id, $message, $photo, $voice_note)
    {
        ...
        /* post notification */
        if ($handle == "comment") {
            $this->post_notification(array('to_user_id' => $parent_comment['author_id'], 'action' => 'reply', 'hub' => "LocalHub", 'node_type' => $parent_comment['node_type'], 'node_url' => $parent_comment['node_id'], 'notify_id' => 'comment_' . $comment['comment_id']));
            if ($post['author_id'] != $parent_comment['author_id']) {
                $this->post_notification(array('to_user_id' => $post['author_id'], 'action' => 'comment', 'hub' => "LocalHub", 'node_type' => $parent_comment['node_type'], 'node_url' => $parent_comment['node_id'], 'notify_id' => 'comment_' . $comment['comment_id']));
            }
        } else {
            $this->post_notification(array('to_user_id' => $post['author_id'], 'action' => 'comment', 'hub' => "LocalHub", 'node_type' => $handle, 'node_url' => $node_id, 'notify_id' => 'comment_' . $comment['comment_id']));
        }
    ...
    public function react_comment($comment_id, $reaction)
    {
        ...
        switch ($comment['node_type']) {
            case 'post':
                $this->post_notification(array('to_user_id' => $comment['author_id'], 'action' => 'react_' . $reaction, 'hub' => "LocalHub", 'node_type' => 'post_comment', 'node_url' => $comment['node_id'], 'notify_id' => 'comment_' . $comment_id));
                break;

            case 'photo':
                $this->post_notification(array('to_user_id' => $comment['author_id'], 'action' => 'react_' . $reaction, 'hub' => "LocalHub", 'node_type' => 'photo_comment', 'node_url' => $comment['node_id'], 'notify_id' => 'comment_' . $comment_id));
                break;

            case 'comment':
                $_node_type = ($comment['parent_comment']['node_type'] == "post") ? "post_reply" : "photo_reply";
                $_node_url = $comment['parent_comment']['node_id'];
                $this->post_notification(array('to_user_id' => $comment['author_id'], 'action' => 'react_' . $reaction, 'hub' => "LocalHub", 'node_type' => $_node_type, 'node_url' => $_node_url, 'notify_id' => 'comment_' . $comment_id));
                break;
        }
    }
    ...
    public function get_photos($id, $type = 'user', $offset = 0, $pass_check = true)
    {
        ...
        switch ($type) {
            ...
            case 'user':
                ...
                $offset *= $system['min_results_even'];
                $picQuery = sprintf("SELECT posts.post_id as pic_post_id, posts_photos.photo_id, posts_photos.source, posts_photos.blur, posts.privacy FROM posts_photos INNER JOIN posts ON posts.post_id = posts_photos.post_id WHERE posts.user_id = %s AND posts.user_type = 'user' AND posts.in_group = '0' AND posts.in_event = '0' ORDER BY posts_photos.photo_id DESC LIMIT %s, %s", secure($id, 'int'), secure($offset, 'int', false), secure($system['min_results_even'], 'int', false));
                $get_photos = $db->query($picQuery) or _error("SQL_ERROR_THROWEN");
                if ($get_photos->num_rows > 0) {
                    while ($photo = $get_photos->fetch_assoc()) {
        ...
        if ($this->_data['friends_ids']) {
            ...
            if ($get_members->num_rows > 0) {
                ...
                if ($invites_list) {
                    ...
                    while ($friend = $get_friends->fetch_assoc()) {
                        $friend['user_picture'] = get_picture($friend['user_picture'], $friend['user_gender']);
                        $friend['user_picture'] = 'includes/wallet-api/image-exist-api.php?userPicture=' . $friend['user_picture'] . '&userPictureFull=' . $system['system_uploads'] . '/' . $friend['user_picture_full'] . '&type=1';
    ...
    public function post_forum_reply($thread_id, $text)
    {
        ...
        /* post notification */
        $this->post_notification(array('to_user_id' => $thread['user_id'], 'action' => 'forum_reply', 'hub' => "LocalHub", 'node_url' => $thread['thread_id'] . '/' . $thread['title_url'] . '/#reply-' . $reply_id));
    ...
    public function send_gift($user_id, $gift_id)
    {
        ...
        /* post new notification */
        $this->post_notification(array('to_user_id' => $user_id, 'action' => 'gift', 'hub' => "LocalHub", 'node_url' => $db->insert_id));
    }
    ...
    public function settings($edit, $args)
    {
        ...
        switch ($edit) {
            ...
            case 'basic':
                ...
                if (!valid_name_no_numbers($args['firstname'])) {
                    throw new Exception(__("Your first name contains invalid characters"));
                }
                if (!valid_names($args['firstname'])) {
                    throw new Exception(__("First name must contain alphabets only"));
                }
                /* validate lastname */
                ...
                if (!valid_name_no_numbers($args['lastname'])) {
                    throw new Exception(__("Your last name contains invalid characters"));
                }
                if (!valid_names($args['lastname'])) {
                    throw new Exception(__("Last name must contain alphabets only"));
                }
                /* validate gender */
            ...
            case 'basicglobal':
                ...
                $get_data = $db->query(sprintf("SELECT * FROM global_profile WHERE user_id = %s", secure($this->_data['user_id'], 'int'))) or _error("SQL_ERROR");
                // echo $get_data->num_rows; echo "<br>";
                // echo $this->_data['user_id'];exit;
                if ($get_data->num_rows > 0) {
                    /* update user */
                    $db->query(sprintf("UPDATE global_profile SET user_firstname = %s, user_lastname = %s, user_gender = %s, user_country = %s, user_birthdate = %s, user_relationship = %s, user_biography = %s, user_website = %s WHERE user_id = %s", secure($args['firstname']), secure($args['lastname']), secure($args['gender']), secure($args['country'], 'int'), secure($args['birth_date']), secure($args['relationship']), secure($args['biography']), secure($args['website']), secure($this->_data['user_id'], 'int'))) or _error("SQL_ERROR_THROWEN");
                } else {
                    $queryRun = sprintf("INSERT INTO global_profile (user_firstname, user_lastname, user_gender, user_country, user_birthdate, user_relationship, user_biography, user_website, user_id) VALUES ( %s, %s, %s, %s, %s, %s, %s, %s, %s)", secure($args['firstname']), secure($args['lastname']), secure($args['gender']), secure($args['country'], 'int'), secure($args['birth_date']), secure($args['relationship']), secure($args['biography']), secure($args['website']), secure($this->_data['user_id'], 'int'));
                    $db->query($queryRun)  or _error("SQL_ERROR_THROWEN");
                }
                break;
    ...
    {
        global $db, $system, $date;
        $myquery = sprintf("select conversation_id from conversations_users WHERE user_id = %s", secure($from_id, 'int'));
        $fromInConv = $db->query($myquery) or _error("SQL_ERROR_THROWEN");
        $fromInConv1 = $fromInConv->fetch_assoc();


        $myquery = sprintf("select conversation_id from conversations_users WHERE user_id = %s", secure($to_id, 'int'));
        $toInConv = $db->query($myquery) or _error("SQL_ERROR_THROWEN");
        $toInConv1 = $toInConv->fetch_assoc();

        if (($fromInConv->num_rows) < 0 && ($toInConv->num_rows < 0) && ($fromInConv1['conversation_id'] !=  $toInConv1['conversation_id'])) {
            /* insert conversation */
            $db->query("INSERT INTO conversations (last_message_id) VALUES ('0')") or _error("SQL_ERROR_THROWEN");
            $conversation_id = $db->insert_id;
            /* insert the sender (viewer) */
            ...
            /* insert recipients */
            //  foreach($recipients as $recipient) {
            $db->query(sprintf("INSERT INTO conversations_users (conversation_id, user_id) VALUES (%s, %s)", secure($conversation_id, 'int'), secure($to_id, 'int'))) or _error("SQL_ERROR_THROWEN");
            //  }
    ...
    function getAllUsers()
    {
        ...
        global $db, $system;
        $countries = [];
        //$get_countriess = $db->query("SELECT `global_user_picture` FROM `users` WHERE `global_user_picture` IS NOT NULL") or _error("SQL_ERROR_THROWEN");
        $get_countriess = $db->query("SELECT `user_picture` FROM `users` WHERE `user_picture` IS NOT NULL ORDER by user_id ASC") or _error("SQL_ERROR_THROWEN");
        if ($get_countriess->num_rows > 0) {
            while ($country = $get_countriess->fetch_assoc()) {
                $countries[] = $country;
            }
        }
        return $countries;
