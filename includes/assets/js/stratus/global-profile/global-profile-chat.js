...
// initialize API URLs
/* chat */
api['chat/live'] = ajax_path + "chat/global-profile-chat.php";
...
api['chat/post'] = ajax_path + "chat/post_global.php";
api['chat/messages'] = ajax_path + "chat/messages_global.php";
api['conversation/check'] = ajax_path + "chat/conversation_global.php?do=check";
api['conversation/get'] = ajax_path + "chat/conversation_global.php?do=get";
...
$(function () {
    $('body').on('click', '.clickChatWindow', function (e) { //alert();
        if ($(this).parent().hasClass("showchatWindow")) {
            $("div.chat-sidebar").removeClass("showchatWindow");
            //$( "div.chat-sidebar-content" ).removeClass( "showchatWindow"); 
            //$( "div.chat-sidebar-footer" ).removeClass( "showchatWindow");       
        } else {
            $("div.chat-sidebar").addClass("showchatWindow");
            //$("div.chat-sidebar-content").addClass("showchatWindow");
            //$("div.chat-sidebar-footer").addClass("showchatWindow");
        }
    })
});
...
function chat_heartbeat() {
    ...
    if (!chat_enabled && window.location.pathname.indexOf("globalMessages") == -1) return;
    ...
    if (window.location.pathname.indexOf("globalMessages") != -1 && $('.panel-messages').data('cid') !== undefined) {
                ...
                if (response.chat_boxes_opened) {
                    $.each(response.chat_boxes_opened, function (i, conversation) {
                        chat_box(
                            ...
                            conversation.multiple_recipients,
                            conversation.link
                        );
                    });
                }
...
$(function () {
    ...
    // chat-box
    $('body').on('click', 'a#js_send_direct_message', function (e) {
        ...
        if (!chat_enabled || $(window).width() < 970) { // Desktops (≥992px)
            ...
            $("textarea.js_post-message").val(value);
        }
    });
    // chat-box
    ...
    $('body').on('click', '.msgGlobal', function (e) {
        if ($(window).width() < 768) {
            $(".mobileMessageWrap").addClass("chatbox").removeClass("users");
            //if($("body.messages_global-localhub").hasClass('_toggle_')){
            $("body.messages_global-localhub").addClass("_toggle_");
            // }
        }
    });
    $('body').on('click', '.js_chat_start', function (e) {
        // if ($(window).width() < 768) {
        // if($("body.messages_global-localhub").hasClass('_toggle_')){
        //     $("body.messages_global-localhub").removeClass("_toggle_");
        //   }
        // }
        if ($(window).width() < 768) {
            $(".mobileMessageWrap").addClass("chatbox").removeClass("users");
        ...
        if (window.location.pathname.indexOf("globalMessages") != -1 && conversation_id) {
            e.preventDefault();
                ...
                if (conversation_id) {
                    ...
                    $.getJSON(api['conversation/check'], { 'uid': user_id }, function (response) {
                        ...
                        if (response.callback) {
                            ...
                            if (response.conversation_id) {
                                window.location = site_path + "/globalMessages/" + response.conversation_id;
                            } else {
                                window.location = site_path + "/globalMessages/new?uid=" + user_id;
                            }
                        }
                    })
                        .fail(function () {
                ...
                if (widget.hasClass("fresh")) {
                    if (window.location.pathname.indexOf("globalMessages") != -1) {
                        ...
                        window.location.replace(
                            site_path + "/globalMessages/" + response.conversation_id
                        );
                    } else {
                        widget.remove();
                        chat_box(
                    ...
                    if (photo === undefined && voice_note === undefined) {
                        ...
                        textarea.focus().val("").height(textarea.css("line-height"));
                        widget.find(".js_scroller:first ul").append(
                            render_template("#chat-message", {
                                message: response.message,
                                image: response.image,
                                voice_note: response.voice_note,
    ...
    $('body').on('click', '.js_delete-conversation', function () {
        confirm(__['Delete Conversation'], __['Are you sure you want to delete this conversation?'], function () {
            $.post(api['chat/reaction'], { 'do': 'delete', 'type': 'globalhub', 'conversation_id': $('.panel-messages').data('cid') }, function (response) {
                if (response.callback) {
                    eval(response.callback);
                }
            }, 'json')
