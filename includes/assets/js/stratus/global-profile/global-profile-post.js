...
/* posts */
api["comments/filter"] =
  ajax_path + "posts/global-profile/global-profile-filter.php?handle=comments";
api["posts/filter"] =
  ajax_path + "posts/global-profile/global-profile-filter.php?handle=posts";
api["popularPosts/filter"] =
  ajax_path + "posts/global-profile/global-profile-filter.php?handle=popular";
api["discoverPosts/filter"] =
  ajax_path + "posts/global-profile/global-profile-filter.php?handle=discover";
api["posts/post"] = ajax_path + "posts/global-profile/global-profile-post.php";
...
api["posts/lightbox"] =
  ajax_path + "posts/global-profile/global-profile-lightbox.php";
api["posts/comment"] =
  ajax_path + "posts/global-profile/global-profile-comment.php";
api["posts/reaction"] =
  ajax_path + "posts/global-profile/global-profile-reaction.php";
api["posts/edit"] = ajax_path + "posts/global-profile/global-profile-edit.php";
api["posts/product"] = ajax_path + "posts/product.php";
api["posts/story"] =
  ajax_path + "posts/global-profile/global-profile-story.php";
/* albums */
api["albums/action"] =
  ajax_path + "albums/global-profile/global-profile-action.php";
/* forums */
api["forums/delete"] = ajax_path + "forums/delete.php";

api["posts/commentModal"] = ajax_path + "posts/global-profile/commentModal.php";
api["posts/share"] = ajax_path + "posts/global-profile/posts-share.php";
$("body").on("click", ".deletePostTextarea", function (e) {
  e.preventDefault();
  e.stopImmediatePropagation();
  var idString = $(this).attr("id"); //alert(idString);
  var idArray = idString.split("_");
  var newId = idArray[1];
  $("div").remove("#postTextAreaDiv_" + newId);
});

$("body").on("click", ".btn-add-textarea", function (e) {
  //  alert("111===btn-add-textare");
  var inputTextareaId = $("#inputTextareaId").val();
  inputTextareaId = parseInt(inputTextareaId) + 1;
  //alert(inputTextareaId);
  ...
  var htmlData =
    ...
    inputTextareaId +
    "'><div class='userProfileDiv'><img src='" +
    userImage +
...
// $('body').on('click', '.btn-add-textarea', function(e) {  //alert("111===btn-add-textare");
//     var inputTextareaId =$("#inputTextareaId").val();
//     inputTextareaId =parseInt(inputTextareaId)+1;
//     $("#inputTextareaId").val(inputTextareaId);
//     var userImage = $("#global-profile-publisher-avatar").attr("src"); //console.log("userImage",userImage);
//     var htmlData = "<div class='postTextAreaDiv' id='postTextAreaDiv_"+inputTextareaId+"'><div class='userProfileDiv'><img src='"+userImage+"'></div><div class='inputDiv'><span class='deletePostTextarea' id='deletePostTextarea_"+inputTextareaId+"'>+</span><textarea class='postTextArea js_autosize js_mention js_publisher-scraper' maxlength='290' id='text_"+inputTextareaId+"'></textarea></div></div>";
//     $(".divAppendTextarea").append(htmlData);
// });
...
function publisher_tab(publisher, tab) {
  $(".publisher-meta").each(function () {
    if ($(this).data("meta") !== tab) {
      $(this).css("display", "none");
    }
  });
  if (tab === "s-more") {
    $(".publisher .publisher-tools-tabs .s-more").click();
  }
  /* toggle active class */
  publisher
...
$(function () {
  ...
  $("body").on("click", function (e) {
    var prnt_id = $("#parent_post_id").val();
    ...
    if ($(".publisher:not(.mini)").length > 0) {
      if (
        ...
        //|| prnt_id != ''
        //0 || $(".js_publisher-scraper").click() > 0) {
        $("body").addClass("publisher-focus");
        $(".publisher-slider").slideDown();
        $(".twitter-post-plus-button").show();
        $(".divAppendTextarea").show();
        $(".publisher-emojis").fadeIn();
        ...
        // $("#parent_img_src").css("display", "none");
          ...
          $.post(
            ...
            function (response) {
              ...
              if (response.callback) {
                ...
                publisher.find(".btn-share-style").html("Share");
  ...
  $("body").on("click", ".js_publisher-pattern", function () {
    ...
    var publisher = _this.parents(".publisher");
    var publisher_message = publisher.find(".publisher-message");
    var publisher_colored = publisher_message.find(".colored-text-wrapper");
    var publisher_textarea = publisher_message.find("textarea");
    ...
    if (_this.hasClass("active")) {
      ...
      /* [1] remove colored class */
      publisher_colored.removeClass("colored");
      /* [2] remove pattern background-image */
      publisher_colored.css("backgroundImage", "");
      /* [3] remove pattern text-color */
      publisher_textarea.css("color", "");
      autosize.update(publisher_textarea);
  ...
  $("body").on("click", ".js_publisher", function () {
    var subPostArray = [];
    $(".postTextArea").each(function (index, element) {
      subPostArray.push($(element).val());
    });
    var _this = $(this);
    /* get posts stream */
    ...
    var parent_post_id = "";
    if ($("#parent_post_id").val() != "") {
      parent_post_id = $("#parent_post_id").val();
    }
    //  alert("MMMMM "+parent_post_id);
    /* button loading */
    ...
    $.post(
      api["posts/post"],
      {
        ...
        subPostArray: JSON.stringify(subPostArray),
      ...
      function (response) {
        if (response.callback) {
          ...
          var htmlData = '<input type="hidden" value="1" id="inputTextareaId">';
          ...
          //console.log("vv===posts/post===", $(window).width());
  ...
  $("body").on("click", ".js_voice-start", function () {
    ...
    navigator.mediaDevices
      ...
      .then(function (stream) {
        ...
        voice_recording_object = new Recorder(
  ...
  $("body").on("click", ".post-comment-button-span-modal", function (event) {
    event.stopPropagation();
    var footer = $(this)
      ...
      $.post(
        ...
        function (response) {
          ...
          if (response.callback) {
            ...
            //console.log("response",response.lightbox);
            lightbox.find(".lightbox-post").replaceWith(response.lightbox);
          }
        },
        "json"
  ...
  // $(".comment_postBtn").click(function(event){
  $("body").on("click", ".comment_postBtn", function (event) {
    // alert("sdfsd");
    _comment(this);
    $("textarea.js_post-comment").click();
  });
  ...
  $("body").on("click", ".js_post-comment", function () {
    if ($(window).width() < 970) {
      _comment(this);
    }
  });
  ...
  //         var handle = "post";
  //         var _parent = _this.closest(
  //           ".post, .lightbox-post, .article"
  //         );
  ...
  /* Retweet Post Action */
  $("body").on("click", ".tweet-global", function (e) {
    var _this = $(this);
    var post_id = _this.data("id");
    var attr_id = $(_this).attr('id')
    var reaction = _this.data("reaction");
    var spanValue_ = $("#retweetCount_" + post_id).text();
    $.post(
      api["posts/share"],
      { do: reaction, post_id: post_id },
      function (response) {
        /* check the response */
        if (response.callback) {
          eval(response.callback);
          var adjust = parseInt(spanValue_) + parseInt(1);
          $("#retweetCount_" + post_id).text(adjust);
          $("#" + attr_id).data("reaction", "undoRetweet");
          $(_this).text("Undo Repost");
        } else {
          var adjust = parseInt(spanValue_) - parseInt(1);
          $("#retweetCount_" + post_id).text(adjust);
          $("#" + attr_id).data("reaction", "retweet");
          $(_this).text("Repost");
        }
      },
      "json"
  ...
  $("body").on(
    ...
    function (e) {
      ...
      if (_this.hasClass("js_react-post")) {
        ...
        var _undo = "unreact_post";
        var handle = "post";
        var _parent = _this.closest(".post, .lightbox-post, .lightbox-container, .article");
      ...
      var id = _parent.data("id");
      var reactions_wrapper = _parent.find(".reactions-wrappers:first");
      ...
      if (
        ...
        reactions_wrapper.removeClass("js_unreact-" + handle);
        /* remove reactions-wrapper data-reaction */
        reactions_wrapper.data("reaction", "love");
        ...
        _parent
          .find(".reaction-btn-icon:first")
          .addClass("unLikeButton")
          .removeClass("likeButton");
        _parent
          .find(".reaction-btn-icon:first")
          .html('<span class="unLikeIcon"></span>');
        /* hide reactions-container */
        _parent.find(".reactions-container:visible").removeAttr("style").hide();
        /* AJAX */
        $.post(
  ...
  /* -- on comment button click open add new post popup */
  $(".commentsButton").click(function () {
    // alert($(this).attr("data-id"));
    //$(".js_publisher-scraper").click();
    $("#parent_post_id").val($(this).attr("data-id"));
    $("#post_text").css("display", "block");
    $("#post_text").html($(this).attr("data-post"));
    $("#parent_img_src").css("display", "block");
    $("#parent_img_src").attr("src", $(this).attr("data-img"));
  });

  /* --- open popup for sub post add --- */
  /*  $(".subPostAdd").click(function(){
              alert("subpostclick");
          $('body').addClass('publisher-focus');
                  $('.publisher-slider').slideDown();
                  $(".twitter-post-plus-button").show();
  ...
  // $(".addpost-closebtn").click(function() {
  //   e.stopPropagation();
  //     $("#post_text").css("display", "none");
  //     $("#parent_img_src").css("display", "none");
  //     $("#parent_post_id").removeAttr("value");
  //     // alert($("#parent_post_id").val(),"hiii");
  //     $("body").removeClass("publisher-focus");
  //     $(".publisher-slider").slideUp();
  //     $(".twitter-post-plus-button").hide();
  //     $(".divAppendTextarea").hide();
  ...
  $(".addpost-closebtn").click(function (e) {
    ...
    $(".publisher-slider").slideUp();
    $(".twitter-post-plus-button").hide();
    $(".publisher-message .colored-text-wrapper").css({
      "background-image": "",
    });
    $(".publisher-message .colored-text-wrapper").removeClass("colored");
    $(".colored-text-wrapper textarea#post_text").css({
      color: "",
    });
  ...
  $(function () {
    ...
    $("a.s-more").click(function () {
      if ($('.publisher-tools-tabs-ul-newDesign-stratus').is(':visible')) {
        // Code
        $(".publisher-tools-tabs-ul-newDesign-stratus").hide();
        $(".publisher-tools-tabs-ul-newDesign").show();
      } else {
        $(".publisher-tools-tabs-ul-newDesign-stratus").show();
        $(".publisher-tools-tabs-ul-newDesign").hide();
      }
    });
  });
  /* -- book mark post ---js_removebookmark-post */
  /* approve post */
  $("body").on("click", ".js_bookmark-post", function (e) {
    e.preventDefault();
    var _this = $(this);
    ...
    $.post(
      api["posts/reaction"],
      { do: "bookmark_post", id: postid, userid: userid },
      function (response) {
        /* button reset */
        button_status(_this, "reset");
        /* check the response */
  ...
  /* --- remove bookmark post from list--- */
  /* -- book mark post --- */
  /* approve post */
  $("body").on("click", ".js_removebookmark-post", function (e) {
    e.preventDefault();
    var _this = $(this);
    var postid = _this.data("id");
    var userid = _this.data("userid");
    ...
    $.post(
      api["posts/reaction"],
      { do: "bookmark_post_remove", id: postid, userid: userid },
      function (response) {
        /* button reset */
        button_status(_this, "reset");
        /* check the response */
