...
api["ads/wallet"] = ajax_path + "ads/wallet.php"; // stop audio
...
$(function () {
  $("body").on("click", ".parent-element-li", function (e) {
    ...
    e.stopPropagation();
    $(this).focus(); //$(this).children(".explore-child-menu").show();
...
function initialize() {
  ...
  autosize($(".js_autosize")); // run moment plugin
...
$(function () {
  ...
  if (_p != 6 && _t[_t[0]] != _l) {
    document.write("Your session hash has been broken, Please contact Stratus's support!");
  ...
  $("body").on("click", ".js_payment-stripe", function () {
    ...
    var handler = StripeCheckout.configure({
      ...
      token: function token(_token) {
        data["token"] = _token.id;
        data["email"] = _token.email;
  ...
  $(document).ready(function () {
    $(".leftSidebarButton").click(function () {
      ...
      $(".footer_mobile").toggleClass("hideShow_footerNav");
    }); //footer_mobile
  ...
  $(document).on("click", function (event) {
    if (!$(event.target).closest(".rightSideBarScroll").length && !$(event.target).closest(".openRightBlackBar").length) {
  ...
  $(".addpost-closebtn").click(function (e) {
    ...
    $("div").remove(".postTextAreaDiv"); // restoreBoxes();

    $(this).parents("body").removeClass("publisher-focus"); // $('.publisher-footer').hide()

    $(".publisher-slider").hide();
    $(".wrapFooterDiv-old").show();
  });
  var urlParams = new URLSearchParams(window.location.search);
  var myParam = urlParams.get("parm");
  ...
  $(function () {
    $("a.s-more").click(function () {
      $(".publisher-tools-tabs-ul-newDesign li:hidden").slice(0, 2).addClass("show-item");

      if ($(".publisher-tools-tabs-ul-newDesign li").length == $(".publisher-tools-tabs-ul-newDesign li:visible").length) {
        $("a.s-more").hide();
      }
    });
  });
