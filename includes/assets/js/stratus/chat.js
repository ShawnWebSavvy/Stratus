...
var chatbox_closing_process = !1,
  chat_calling_process = !1,
  chat_audiocall_ringing_process = !1,
  chat_videocall_ringing_process = !1,
  chat_incall_process = !1;
function reconstruct_chat_widgets() {
  $(window).width() < 970 ||
    $(".chat-widget").each(function (e) {
      $(this).attr("style", ""),
        (e += 1),
        (offset = 210 * e + 10 * e),
        $(this).prevAll(".chat-box").length > 0 &&
        (offset += 50 * $(this).prevAll(".chat-box").length),
        "RTL" == $("html").attr("dir")
...
function color_chat_box(e, a) {
  e.data("color", a),
    e.find(".js_chat-color-me").each(function () {
...
function chat_heartbeat() {
  ...
  else if (chat_enabled || -1 != window.location.pathname.indexOf("messages")) {
    var chat_boxes_opened_client = {};
    if (
      ($.each($(".chat-box:not(.fresh)"), function (e, a) {
        $(a).data("sending") ||
          (chat_boxes_opened_client[$(a).data("cid")] = $(a)
            .find(".conversation:last")
            .attr("id"));
      }),
        -1 != window.location.pathname.indexOf("messages") &&
        void 0 !== $(".panel-messages").data("cid"))
    ) {
      var opened_thread = {};
      $(".panel-messages").data("sending") ||
        ((opened_thread.conversation_id = $(".panel-messages").data("cid")),
          (opened_thread.last_message_id = $(".panel-messages")
            .find(".conversation:last")
            .attr("id")));
      var data = {
        chat_enabled: $("body").data("chat-enabled"),
        chat_boxes_opened_client: JSON.stringify(chat_boxes_opened_client),
        opened_thread: JSON.stringify(opened_thread),
      };
    } else
      var data = {
        chat_enabled: $("body").data("chat-enabled"),
      ...
      function (response) {
        if (response.callback) eval(response.callback);
          ...
          1 == response.has_audiocall
            ...
            $.post(
              api["chat/reaction"],
              { do: "seen", ids: updated_seen_conversations },
              function (response) {
...
function chat_incall_heartbeat(e, a) {
  0 != chat_incall_process &&
    (setTimeout(function () {
      chat_incall_heartbeat(e, a);
    }, min_chat_heartbeat),
      $.post(api["chat/call"], { do: "update_call", type: e, id: a }, "json"));
}
function init_Twilio(e, a, s, t) {
  var o = "video" == e;
  function i(a) {
    const s = document.createElement("div");
    (s.id = a.sid),
      a.on("trackAdded", (e) => c(s, e)),
      a.tracks.forEach((e) => c(s, e)),
      a.on("trackRemoved", l),
      $(".twilio-stream").html(s),
      "video" == e &&
      navigator.getUserMedia &&
      (navigator.mediaDevices
        .getUserMedia({ audio: !1, video: !0 })
        .then((e) => {
          $(".twilio-stream-local")[0].srcObject = e;
        }),
        $(".twilio-stream-local").show());
  }
  function n(a) {
    a.tracks.forEach(l),
      document.getElementById(a.sid).remove(),
      $.post(api["chat/call"], { do: "decline_call", type: e, id: t }, "json"),
      alert(__["Connection has been lost"]),
      window.location.reload();
  }
  function c(e, a) {
    e.appendChild(a.attach());
  }
  function l(e) {
    e.detach().forEach((e) => e.remove());
  }
  (navigator.getUserMedia =
    navigator.getUserMedia ||
    ...
    Twilio.Video.connect(a, { name: s, audio: !0, video: o }).then((e) => {
      e.participants.forEach(i),
        e.on("participantConnected", i),
        e.on("participantDisconnected", n),
        e.once("disconnected", (a) => e.participants.forEach(n)),
...
$(function () {
  $("body").on("click", ".clickChatWindow", function (e) {
    $(this).parent().hasClass("showchatWindow")
      ? $("div.chat-sidebar").removeClass("showchatWindow")
      : $("div.chat-sidebar").addClass("showchatWindow");
  });
}),
  $(function () {
    ...
    function check_calling_response(type, call_id) {
      0 != chat_calling_process &&
        $.post(
          api["chat/call"],
          { do: "check_calling_response", type: type, id: call_id },
          function (response) {
            if (response.callback) eval(response.callback);
            else
    ...
    setTimeout("chat_heartbeat()", 1e3),
      ...
      $("body").on("keyup", ".js_chat-search", function (e) {
      ...
      $("body").on("click", ".backBtn", function (e) {
        $(window).width() < 768 &&
          $(".mobileMessageWrap").addClass("users").removeClass("chatbox");
      }),
      $("body").on("click", ".js_chat_start", function (e) {
        $(window).width() < 768 &&
          ($(".mobileMessageWrap").addClass("chatbox").removeClass("users"),
            $("body.messages-localhub").hasClass("_toggle_") &&
            $("body.messages-localhub").removeClass("_toggle_"));
        var user_id = $(this).data("uid") || !1,
          conversation_id = $(this).data("cid") || !1,
          name = $(this).data("name"),
        ...
        else if (!chat_enabled || $(window).width() < 970) {
                  ...
                  (response.callback
                    ...
                    : response.conversation_id
                      ? (window.location =
                        site_path + "/messages/" + response.conversation_id)
                      : (window.location =
                        site_path + "/messages/new?uid=" + user_id));
              }
            ).fail(function () {
              modal("#modal-message", {
                title: __.Error,
      ...
      $("body").on("keydown", "textarea.js_post-message", function (e) {
        $(window).width() >= 970 &&
          13 == e.keyCode &&
          0 == e.shiftKey &&
          (e.preventDefault(), _post_message(this));
      }),
      $("body").on("click", "li.js_post-message", function (e) {
        _post_message(this);
      }),
      $("body").on("click", ".js_chat-attachment-remover", function () {
        var e = $(this).parents(".chat-widget, .panel-messages"),
          a = e.find(".chat-attachments"),
          s = $(this).parents("li.item");
        e.removeData("photo"),
          s.remove(),
          a.hide(),
          e.find(".x-form-tools-attach").show(),
          e.find(".x-form-tools-voice").show();
      }),
      $("body").on(
        "keyup paste change input propertychange",
        "textarea.js_post-message",
        function () {
          if (chat_typing_enabled) {
            var _this = $(this),
              widget = _this.parents(".chat-widget, .panel-messages"),
              conversation_id = widget.data("cid") || !1,
              is_typing = _this.val() ? 1 : 0;
            conversation_id &&
      ...
      $("body").on("click", ".js_chat-box-close", function () {
        ...
        widget.remove(),
          reconstruct_chat_widgets(),
          (chatbox_closing_process = !0),
          void 0 !== widget.data("cid") &&
          $.post(
            api["chat/reaction"],
            { do: "close", conversation_id: widget.data("cid") },
            function (response) {
              response.callback && eval(response.callback),
                (chatbox_closing_process = !1);
            },
            "json"
          ).fail(function () {
            modal("#modal-message", {
        ...
        confirm(
          ...
          function () {
            $.post(
              ...
              {
                do: "delete",
                conversation_id: $(".panel-messages").data("cid"),
              },
              function (response) {
                response.callback && eval(response.callback);
              },
      ...
      color_chat_box(
        $(".panel-messages"),
        $(".panel-messages").attr("data-color")
      ),
      $("body").on("click", ".js_chat-colors-menu-toggle", function () {
        0 == $(this).parent().find(".chat-colors-menu").length &&
          $(this).after(render_template("#chat-colors-menu")),
          $(this).parent().find(".chat-colors-menu").toggle();
      }),
      $("body").on("click", function (e) {
        $(e.target).hasClass("js_chat-colors-menu-toggle") ||
          $(e.target).parents(".js_chat-colors-menu-toggle").length > 0 ||
          $(e.target).hasClass("chat-colors-menu") ||
          $(e.target).parents(".chat-colors-menu").length > 0 ||
      ...
      $("body").on("click", ".js_chat-color", function () {
        ...
        color_chat_box(chat_widget, color),
          ...
          $.post(
            api["chat/reaction"],
            { do: "color", conversation_id: conversation_id, color: color },
            function (response) {
              response.callback && eval(response.callback);
            },
            "json"
      ...
      $("body").on("click", ".js_chat-call-start", function () {
        ...
        modal(
          "#chat-calling",
          { type: type, is_video: is_video, is_audio: is_audio, name: name },
          is_video ? "large" : "default"
        ),
          $.post(
            api["chat/call"],
            { do: "create_call", type: type, user_id: user_id },
            function (response) {
              response.callback
                ? eval(response.callback)
                : 0 == response.call_id
                  ...
                  : "recipient_offline" == response.call_id
                    ? ($(".js_chat-call-close").show(),
                      $(".js_chat-calling-message").html(
                        "<span style='color: red'>" + __["is Offline"] + "</span>"
                      ))
                    : "recipient_busy" == response.call_id
                      ? ($(".js_chat-call-close").show(),
                        $(".js_chat-calling-message").html(
                          "<span style='color: red'>" + __["is Busy"] + "</span>"
                        ))
                      : "caller_busy" == response.call_id
                        ? ($(".js_chat-call-close").show(),
                          $(".js_chat-calling-message").html(
                            "<span style='color: red'>" +
      ...
      $("body").on(
        "click",
        ".js_chat-call-cancel, .js_chat-call-decline, .js_chat-call-end",
        function () {
          var type = $(this).data("type"),
            id = $(this).data("id"),
            reload = !1;
          $(this).hasClass("js_chat-call-cancel") &&
            ((chat_calling_process = !1),
              $("#chat-calling-sound")[0].stop(),
              clearTimeout(end_call)),
            $(this).hasClass("js_chat-call-decline") &&
            $("#chat-ringing-sound")[0].stop(),
            $(this).hasClass("js_chat-call-end") && (reload = !0),
            $.post(
              api["chat/call"],
              { do: "decline_call", type: type, id: id },
              function (response) {
                response.callback && eval(response.callback),
                  reload && (window.location.href = site_path);
              },
              "json"
            ).fail(function () {
              modal("#modal-message", {
      ...
      $("body").on("click", ".js_chat-call-answer", function () {
        ...
        $.post(
          api["chat/call"],
          { do: "answer_call", type: type, id: id },
          function (response) {
            if (response.callback) eval(response.callback);
            else {
              "audio" == type && (chat_audiocall_ringing_process = !1),
                "video" == type && (chat_videocall_ringing_process = !1),
                (chat_incall_process = !0),
  ...
  $("body").on("click", ".msgLocal", function (e) {
    $(window).width() < 768 &&
      ($(".mobileMessageWrap").addClass("chatbox").removeClass("users"),
        $("body.messages-localhub").addClass("_toggle_"));
  });
