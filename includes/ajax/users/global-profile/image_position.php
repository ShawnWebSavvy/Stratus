...
try {
    ...
    switch ($_POST['handle']) {
        case 'user':
            ...
            /* get full picture */
            $sleQ = sprintf("SELECT posts_photos.source FROM users INNER JOIN global_posts_photos as posts_photos ON users.global_user_cover_id = posts_photos.photo_id WHERE users.user_id = %s", secure($user->_data['user_id'], 'int'));
            $get_picture = $db->query($sleQ) or _error("SQL_ERROR_THROWEN");
            ...
            $table_name = "users";
            $table_cover_field = "global_user_cover";
        ...
        case 'page':
            ...
            $get_page = $db->query(sprintf("SELECT posts_photos.source FROM pages INNER JOIN global_posts_photos as posts_photos ON pages.page_cover_id = posts_photos.photo_id WHERE pages.page_id = %s", secure($_POST['id'], 'int'))) or _error("SQL_ERROR_THROWEN");
        ...
        case 'group':
            ...
            $get_group = $db->query(sprintf("SELECT posts_photos.source FROM `groups` INNER JOIN global_posts_photos as posts_photos ON `groups`.group_cover_id = posts_photos.photo_id WHERE `groups`.group_id = %s", secure($_POST['id'], 'int'))) or _error("SQL_ERROR_THROWEN");
        ...
        case 'event':
            ...
            $get_event = $db->query(sprintf("SELECT posts_photos.source FROM `events` INNER JOIN global_posts_photos as posts_photos ON `events`.event_cover_id = posts_photos.photo_id WHERE `events`.event_id = %s", secure($_POST['id'], 'int'))) or _error("SQL_ERROR_THROWEN");
    ...
    // update cover photo
    $queryCover = sprintf("UPDATE %s SET %s = %s, %s = %s WHERE %s = %s", $table_name, $table_cover_field, secure($image_name), $table_position_field, secure($_POST['position']), $table_id_field, secure($_POST['id'], 'int'));
    $db->query($queryCover) or _error("SQL_ERROR_THROWEN");
