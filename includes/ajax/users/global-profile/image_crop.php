...
try {
    ...
    switch ($_POST['handle']) {
        ...
        case 'page':
            ...
            $get_page = $db->query(sprintf("SELECT posts_photos.source FROM pages INNER JOIN global_posts_photos as posts_photos ON pages.page_picture_id = posts_photos.photo_id WHERE pages.page_id = %s", secure($_POST['id'], 'int'))) or _error("SQL_ERROR_THROWEN");
        ...
        case 'group':
            ...
            $get_group = $db->query(sprintf("SELECT posts_photos.source FROM `groups` INNER JOIN global_posts_photos as posts_photos ON `groups`.group_picture_id = posts_photos.photo_id WHERE `groups`.group_id = %s", secure($_POST['id'], 'int'))) or _error("SQL_ERROR_THROWEN");
    ...
    $updateCrop = sprintf("UPDATE %s SET %s = %s WHERE %s = %s", $table_name, $table_picture_field, secure($image_name), $table_id_field, secure($_POST['id'], 'int'));
    //exit;
    $db->query($updateCrop) or _error("SQL_ERROR_THROWEN");
