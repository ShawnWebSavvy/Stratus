...
try {
    ...
    switch ($_POST['handle']) {
        case 'cover-user':
            /* update user cover */
            $db->query(sprintf("UPDATE users SET global_user_cover = null, global_user_cover_id = null, user_cover_position = null WHERE user_id = %s", secure($user->_data['user_id'], 'int'))) or _error("SQL_ERROR_THROWEN");
            break;

        case 'picture-user':
            /* update user picture */
            $db->query(sprintf("UPDATE users SET global_user_picture = null, global_user_picture_id = null WHERE user_id = %s", secure($user->_data['user_id'], 'int'))) or _error("SQL_ERROR_THROWEN");
            /* return */
