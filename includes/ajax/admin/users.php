...
try {
	...
	switch ($_GET['do']) {
		case 'edit_account':
			...
			$checkExistUser  =  $user->httpGetCurl('/users/whitelabel/get-user-info/' . $user_info['user_email']);
			if ($user_info['user_email'] != $_POST['user_email']) {
				if (isset($checkExistUser['userId'])) {
					$updateEmailResponse  =  $user->httpPostCurl('/users/whitelabel/update-email', array("email" => $_POST['user_email'], "userId" => $checkExistUser['userId']));
					if ($updateEmailResponse['message'] != "Email updated successfully") {
						throw new Exception(__("Sorry, it looks like") . " <strong>" . $_POST['user_email'] . "</strong> " . __("unable to update."));
					} else {
						/* email update */
