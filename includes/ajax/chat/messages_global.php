...
try {
	...
	if(isset($_GET['user_id'])) {
		$return['user_online'] = ($userGlobal->user_online($_GET['ids']))? true: false;
	}

	/* if conversation_id not set -> check if there is a mutual conversation */
	if(!isset($_GET['conversation_id'])) {
		$mutual_conversation = $userGlobal->get_mutual_conversation((array)$_GET['user_id']);
	...
	$conversation = $userGlobal->get_conversation($_GET['conversation_id']);

	/* get conversation messages */
	$conversation['messages'] = $userGlobal->get_conversation_messages($_GET['conversation_id']);
	/* check if last message sent by the viewer */
	if($conversation['seen_name_list'] && end($conversation['messages'])['user_id'] == $userGlobal->_data['user_id']) {
