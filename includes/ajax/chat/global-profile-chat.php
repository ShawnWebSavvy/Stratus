...
try {
	...
	if ($system['chat_enabled'] && !($detect->isMobile() && !$detect->isTablet())) {
		...
		/* get online friends */
		$online_friends = $userGlobal->get_online_friends();
		/* get offline friends */
		$offline_friends = $userGlobal->get_offline_friends();
		/* get sidebar friends */
		...
		if ($userGlobal->_data['user_chat_enabled']) {
			/* return */
			$return['master']['chat_enabled'] = '1';
			$return['master']['online_friends_count'] = count($online_friends);
		} elseif (!$userGlobal->_data['user_chat_enabled'] && $_GET['chat_enabled']) {
			/* return */
		...
		if (!(empty($chat_boxes_opened_client) && empty($_SESSION['chat_boxes_opened']))) {
			...
			foreach ($chat_boxes_pre_opened as $opened_conversation_id) {
				/* get conversation */
				$conversation = $userGlobal->get_conversation($opened_conversation_id);
			...
			foreach ($chat_boxes_pre_updated as $updated_conversation_id) {
				/* get conversation */
				$conversation = $userGlobal->get_conversation($updated_conversation_id);
				if ($conversation) {
					$return_this = false;
					...
					if ($conversation['last_message_id'] != $chat_boxes_opened_client[$conversation['conversation_id']]) {
						$return_this = true;
						/* get new messages */
						$messages = $userGlobal->get_conversation_messages($conversation['conversation_id'], 0, $chat_boxes_opened_client[$conversation['conversation_id']]);
						/* assign variables */
						$smarty->assign('messages', $messages);
						/* return */
						$last_message = end($messages);
						$conversation['is_me'] = ($last_message['user_id'] == $userGlobal->_data['user_id']) ? true : false;
					...
					if (!$conversation['multiple_recipients']) {
						...
						$conversation['user_online'] = ($userGlobal->user_online($conversation['recipients'][0]['user_id'])) ? true : false;
					}
					/* return */
					if ($return_this) {
		...
		$chat_boxes_new = $userGlobal->get_conversations_new();
	...
	if (isset($_POST['opened_thread'])) {
		/* get conversation */
		$conversation = $userGlobal->get_conversation($_POST['opened_thread']['conversation_id']);
		if ($conversation) {
			$return_this = false;
			...
			if ($conversation['last_message_id'] != $_POST['opened_thread']['last_message_id']) {
				$return_this = true;
				/* get new messages */
				$messages = $userGlobal->get_conversation_messages($conversation['conversation_id'], 0, $_POST['opened_thread']['last_message_id']);
				/* assign variables */
				$smarty->assign('messages', $messages);
				/* return */
				$last_message = end($messages);
	...
	if ($system['audio_call_enabled']) {
		...
		$audiocall = $userGlobal->check_new_calls("audio");
		if ($audiocall) {
	...
	if ($system['video_call_enabled'] && !$return['has_audiocall']) {
		...
		$videocall = $userGlobal->check_new_calls("video");
		if ($videocall) {
