	...
	switch ($_POST['do']) {
		case 'delete_post':
			...
			if ($refresh) {
				/* return */
				if ($refresh['callback']) {
					$return['callback'] = $refresh['callback'];
				} else {
					$return['refresh'] = true;
				}
			}
		...
		case 'boost_post':
			...
			modal("SUCCESS", __("Success"),  __("This post has been boosted Successfully on your timeline"));
			break;

		case 'unboost_post':
			// unboost post
			$user->unboost_post($_POST['id']);
			modal("SUCCESS", __("Success"),  __("This post has been unboost Successfully from your timeline"));
			break;

		case 'pin_post':
			// pin post
			$user->pin_post($_POST['id']);
			modal("SUCCESS", __("Success"),  __("This post has been Pinned Successfully on your profile"));
			break;

		case 'unpin_post':
			// unpin post
			$user->unpin_post($_POST['id']);
			modal("SUCCESS", __("Success"),  __("This post has been unpinned Successfully from your profile"));
			break;
