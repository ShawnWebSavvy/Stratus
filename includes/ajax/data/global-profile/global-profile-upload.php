            ...
            if ($_POST["multiple"] == "true") {
                ...
                foreach ($files as $file) {
                    ...
                    if ($system['s3_enabled']) {
                        ...
                        $source->store(array(
                            "service" => "s3",
                            "aws_access_key_id" => $system['s3_key'],
                            "aws_secret_access_key" => $system['s3_secret'],
                            "region" => $system['s3_region'],
                            "headers" => array("Cache-Control" => "max-age=31536000, public"),
                            "path" => $system['s3_bucket'] . "/uploads/" . $file_name
                        ));
                ...
                switch ($_POST['handle']) {
                    case 'cover-user':
                        ...
                        $db->query(sprintf("INSERT INTO global_posts_photos (post_id, album_id, source, blur) VALUES (%s, %s, %s, %s)", secure($post_id, 'int'), secure($user->_data['user_album_covers'], 'int'), secure($file_name), secure($image_blured))) or _error("SQL_ERROR_THROWEN");
                        $photo_id = $db->insert_id;
                        /* update user cover */
                        $db->query(sprintf("UPDATE users SET global_user_cover = %s, global_user_cover_id = %s WHERE user_id = %s", secure($file_name), secure($photo_id, 'int'), secure($user->_data['user_id'], 'int'))) or _error("SQL_ERROR_THROWEN");
                        break;
                    ...
                    case 'picture-user':
                        ...
                        $db->query(sprintf("INSERT INTO global_posts_photos (post_id, album_id, source, blur) VALUES (%s, %s, %s, %s)", secure($post_id, 'int'), secure($user->_data['user_album_pictures'], 'int'), secure($file_name), secure($image_blured))) or _error("SQL_ERROR_THROWEN");
                        $photo_id = $db->insert_id;
