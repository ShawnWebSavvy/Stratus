...
class Message
{
    ...
    public function get_mutual_conversation($recipients,$check_deleted = false)
    {
        ...
        {
           echo $this->_data['user_id'] = $recipients[0];
        }
    ...
    public function get_conversation_messages($conversation_id,$offset = 0, $last_message_id = null)
    {
        ...
        //echo sprintf("SELECT COUNT(*) as count FROM conversations_users WHERE conversation_id = %s AND user_id = %s", secure($conversation_id, 'int'), secure($this->_data['user_id'], 'int'));
        if($conversation_id){
    ...
    public function get_post($post_id, $get_comments = true, $pass_privacy_check = false)
    {
        ...
        switch ($post['post_type']) {
            ...
            case 'video':
                ...
                // var_dump($post['video']['thumbnail']); die;

                //$post['video']['thumbnail'] = $system['system_url'] . '/includes/wallet-api/get-picture-api.php?picture=' . $system['system_uploads'] . '/' . $post['video']['thumbnail'] . '&type=movie';
    ...
    public function comment($handle, $node_id, $message, $photo, $voice_note)
    {
        ...
        switch ($handle) {
            ...
            case 'comment':
                ...
                $post_id = $parent_comment['node_id'];
                break;
        }
	...
	function createChatBoxMessage($chat_user,$chat_box_message,$conversation_id,$photo,$voice_note,$recipients) {
	...
	/* add conversation to opened chat boxes session if not */
		...
		$conversation = $this->get_conversation($conversation_id);
		$online_friends = $this->get_online_friends();
		
		/* get offline friends */
		$offline_friends = $this->get_offline_friends();
