...
try {
	...
	switch ($_GET['view']) {
		case '':
			...
			$smarty->assign('subactive_page', 'settings');
		...
		case 'profile':
			...
			switch ($_GET['sub_view']) {
				...
				case 'global':
					// parse birthdate
					$userGlobal->_data['user_birthdate_parsed'] = date_parse($userGlobal->_data1['user_birthdate']);

					// get countries
					$countries = $user->get_countries();
					/* assign variables */
					// echo "<pre>";
					// print_r($userGlobal);die;
					$smarty->assign('countries', $countries);
					$smarty->assign('userGlobal', $userGlobal);
					$smarty->assign('active_page', 'GlobalHub');
					$smarty->assign('subactive_page', 'globalhub_settings');
