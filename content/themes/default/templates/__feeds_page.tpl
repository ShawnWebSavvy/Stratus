    ...
    <div class="ui-box uiBoxPosition">
        <div class="img">
            <a href="{$system['system_url']}/pages/{$_page['page_name']}{if $_search}?ref=qs{/if}">
                <img class="lazyload" alt="{$_page['page_title']}" data-src="{$_page['page_picture']}" />
            </a>
        </div>
        <div class="mt10">
            <span class="js_user-popover" data-uid="{$_page['page_id']}" data-type="page">
                <a class="h6"
                    href="{$system['system_url']}/pages/{$_page['page_name']}{if $_search}?ref=qs{/if}">{$_page['page_title']}</a>
            </span>
        ...
        <div class="mt10">
            {if $_page['i_like']}
            <button type="button" class="btn js_unlike-page unlike-button-stratus" data-id="{$_page['page_id']}">
                <img class="lazyload" data-src="{$system['system_uploads_assets']}/content/themes/default/images/svg/svgImg/local_hubN.svg"
                    class="whiteicon">{__("Unlike")}
            </button>
            {else}
            <button type="button" class="btn js_like-page like-button-stratus" data-id="{$_page['page_id']}">
                <img class="lazyload" data-src="{$system['system_uploads_assets']}/content/themes/default/images/svg/svgImg/local_hubN.svg"
                    class="whiteicon">
                {__("Like")}
            </button>
...
<li class="feeds-item">
    <div class="data-container {if $_small}small{/if}">
        <a class="data-avatar" href="{$system['system_url']}/pages/{$_page['page_name']}{if $_search}?ref=qs{/if}">
            <img class="lazyload" data-src="{$_page['page_picture']}" alt="{$_page['page_title']}">
        </a>
