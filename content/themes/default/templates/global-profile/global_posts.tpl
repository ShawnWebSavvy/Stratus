...
<style>
	@media screen and (min-width: 700px) {
		.bricklayer-column-sizer {
			/* If page is greater than 700px, columns will be 50% wide.  */
			width: 50%;
		}
	}
</style>
...
<style>
	@media screen and (min-width: 700px) {
		.bricklayer-column-sizer {
			/* If page is greater than 700px, columns will be 100% wide.  */
			width: 100%;
		}
	}
</style>
{/if}
<div class="js_posts_stream" data-get="{$_get}" data-filter="{if $_filter}{$_filter}{else}all{/if}" {if
	$_id}data-id="{$_id}" {/if}>
	{if $posts}
	<div class="bricklayer">
		<!-- posts -->
		{foreach $posts as $post}
		{include file='global-profile/global-profile__feeds_posts.tpl' _get=$_get}
		{/foreach}
		<!-- posts -->
	</div>
	...
	<!-- see-more -->
	{else}
	<div class="" data-get="{$_get}" data-filter="{if $_filter}{$_filter}{else}all{/if}" {if $_id}data-id="{$_id}"
		{/if}>
