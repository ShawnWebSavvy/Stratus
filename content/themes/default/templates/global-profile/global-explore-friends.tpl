<div class="card bg-transparent _user__data _request_sec">
    {if $view == ""}
        ...
        <div class="card-body pd0">
            {if $people}
                <ul class="requstSectionwrap exploreFriendsSection">
                    {foreach $people as $_user}
                    {include file='global-profile/__feeds_user.tpl' _tpl="list" _connection="follow"} {/foreach}
                </ul>
            ...
            {else}
                ...
                <div class="__no_data_contet__ text-center">
                    <p class="text-center text-muted">
                        <img src="{$system['system_url']}/content/themes/default/images/no_results16.jpg" style="width: 100%;"
                            alt="No friends">
        ...
        <div class="card-body ">
            {if count($people) > 0}
                <ul class="requstSectionwrap">
                    {foreach $people as $_user} {include file='__feeds_user.tpl' _tpl="list" _connection=$_user['connection']}
                    {/foreach}
                </ul>
            {else}
                <p class="text-center text-muted">
        ...
        <div class="card-body">
            {if $user->_data['friend_requests']}
                <ul class="requstSectionwrap">
                    ...
                    {include file='__feeds_user.tpl' _tpl="list" _connection="request"} {/foreach}
                </ul>
            {else}
                <p class="text-center text-muted">
                    {__("No new requests")}
        ...
        <div class="card-body">
            {if $user->_data['friend_requests_sent']}
                <ul class="requstSectionwrap">
                    ...
                    {include file='__feeds_user.tpl' _tpl="list" _connection="cancel"} {/foreach}
                </ul>
            {else}
                <p class="text-center text-muted">
                    {__("No new requests")}
