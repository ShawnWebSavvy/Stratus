    ...
    <div class="ui-box uiBoxPosition">
        <div class="img">
            <a href="{$system['system_url']}/groups/{$_group['group_name']}{if $_search}?ref=qs{/if}">
                <img class="lazyload" alt="{$_group['group_title']}" data-src="{$_group['group_picture']}" />
            </a>
        </div>
        <div class="mt10">
            <a class="h6"
        ...
        <div class="mt10">
            ...
            <button type="button" class="btn btn-success {if !$_no_action}btn-delete{/if} js_leave-group"
                data-id="{$_group['group_id']}" data-privacy="{$_group['group_privacy']}">
                <i class="fa fa-check mr5"></i>{__("Joined")}
            </button>
            {elseif $_group['i_joined'] == "pending"}
            <button type="button" class="btn btn-warning js_leave-group" data-id="{$_group['group_id']}"
                data-privacy="{$_group['group_privacy']}">
                <i class="fa fa-clock mr5"></i>{__("Pending")}
            </button>
            {else}
            <button type="button" class="btn  btn-success js_join-group" data-id="{$_group['group_id']}"
                data-privacy="{if $user->_data['user_id'] == $_group['group_admin']}public{else}{$_group['group_privacy']}{/if}">
                <!-- <img class="lazyload" data-src="{$system['system_uploads_assets']}/content/themes/default/images/svg/svgImg/add_friend_icon.svg"> -->
...
<li class="feeds-item">
    <div class="data-container {if $_small}small{/if}">
        <a class="data-avatar" href="{$system['system_url']}/groups/{$_group['group_name']}{if $_search}?ref=qs{/if}">
            <img class="lazyload" data-src="{$_group['group_picture']}" alt="{$_group['group_title']}">
        </a>
            ...
            <div class="float-right">
                ...
                <button type="button" class="btn  btn-success js_join-group" data-id="{$_group['group_id']}"
                    ...
                    <img class="btn_image_ lazyload"
                        data-src="{$system['system_uploads_assets']}/content/themes/default/images/svg/svgImg/add_friend_icon.svg">
                    <img class="btn_image_hover "
                        src="{$system['system_uploads_assets']}/content/themes/default/images/svg/svgImg/add_friend-hover.svg">
                    {__("Join")}
                </button>
