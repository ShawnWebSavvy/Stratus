...
<div class="col-sm-6 col-md-4 ">
    <a href="{$system['system_url']}/blogs/{$article['post_id']}/{$article['article']['title_url']}"
        class="blog-container {if $_iteration == 1}primarys{/if}">
        <div class="blog-image">
            <img class="lazyload" data-src="{$article['article']['parsed_cover']}">
        </div>
        ...
        <div class="blog-more">
            <span class="btn btn-info rounded-pill">{__("More")}</span>
        </div>
    </a>
</div>
{else}
<div class="post-media list">
    <div class="post-media-image-wrapper">
        <a class="post-media-image"
            href="{$system['system_url']}/blogs/{$article['post_id']}/{$article['article']['title_url']}">
            <div
                style="padding-top: 50%; background-position: center center; background-size: cover; background-image:url('{$article['article']['parsed_cover']}');">
            </div>
        </a>
        <div class="post-media-image-meta">
            <a class="article-category {if $_small}small{/if}"
    ...
    <div class="post-media-meta">
        <a class="title mb5"
            href="{$system['system_url']}/blogs/{$article['post_id']}/{$article['article']['title_url']}">{$article['article']['title']}</a>
        <div class="text mb5">
            {if $_small}
