{if !$standalone}<div class="carsds" data-id="{$post['post_id']}" post-type="{$post['post_type']}">{/if}
    ...
    <div class='post{if $_get == " posts_profile" && $user->_data[' user_id']==$post['author_id'] && ($post['is_hidden']
            ...
            <div class="post-actions clearfix">
                ...
                <div class="_share_post_ dropdown">
                    <div class="action-btn dropdown-toggle dropdown-toggle-share" id="stratus_post_{$post['post_id']}"
                        parent-data-id="{$post['post_id']}" data-toggle="dropdown" aria-haspopup="true"
                        aria-expanded="false" data-url="posts/share.php?do=create&post_id={$post['post_id']}">
        ...
        <div class="post-approval">
            <button class="btn btn-success  mr5 js_approve-post"><i class="fa fa-check mr5"></i>{__("Approve")}</button>
