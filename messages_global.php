...
try {
	...
	if ($view == 'message') {
		if (!isset($_GET['cid'])) {
			if ($userGlobal->_data['conversations']) {
				// echo "<pre>"; print_r($userGlobal->_data['conversations']);
				// die;
				$conversation = $userGlobal->_data['conversations'][0];
				$conversation['messages'] = $userGlobal->get_conversation_messages($conversation['conversation_id']);
			}
		} else {
			/* check cid is valid */
			...
			$conversation = $userGlobal->get_conversation($_GET['cid']);
			$conversation['messages'] = $userGlobal->get_conversation_messages($conversation['conversation_id']);
		}
		// assign variables
	...
	$smarty->assign('subactive_page', 'globalhub_messages');
} catch (Exception $e) {
	_error(__("Error"), $e->getMessage());
}

// page footer
page_footer("messages_global");
